...
exports.init = async function (ctx) {
    ...
    action_TIMELINE_NOTIFICATION.registerRunListener(async (args) => {
        ...
        await sleep(4000);
        const updatedArgs = action_TIMELINE_NOTIFICATION.getArgumentValues()
        const { message } = updatedArgs;

        ctx.homey.app.log('[action_TIMELINE_NOTIFICATION] - delayed message', message);

        await ctx.homey.app.action_TIMELINE_NOTIFICATION(message, );
    });
};
